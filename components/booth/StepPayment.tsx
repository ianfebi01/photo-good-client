...
// ── Hook: polling — starts/stops automatically, no manual refs ────
...
function usePaymentPolling( orderId: string | null ): PollResult {
  ...
  useEffect( () => {
    ...
    // Reset to 'polling' so stale terminal/settlement results from
    // a previous charge don't leak into a new one.
    ...
    // Fire the first poll immediately so we don't wait POLL_INTERVAL
    poll()
...
function resolveInitialPhase(): { phase: Phase; charge: Charge | null } {
  ...
  // Already have a pending charge in the store → restore it
  ...
  // Fresh session (after reset) → show idle/welcome screen
...
export function StepPayment() {
  const { sessionId, setPayment, advanceToCapture } = useBoothStore()

  // Compute initial values once on mount (not on every render)
  ...
  // ── Create charge (called explicitly, not on mount) ──────────────
  ...
  // ── Advance when countdown hits 0 ────────────────────────────────
  useEffect( () => {
    if ( phase === 'paid' && countdown === 0 ) {
      advanceToCapture()
    }
  }, [phase, countdown, advanceToCapture] )
  ...
  const handleRetry = useCallback( () => {
    // Clear stale payment state from the store so createCharge
    // starts fresh.
  ...
  if ( phase === 'pending' && charge ) {
    return (
      <div className="container px-4 py-8 mx-auto overflow-auto grow scrollbar-none">
  ...
  if ( phase === 'paid' ) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-6 px-4 animate-in fade-in duration-500">
